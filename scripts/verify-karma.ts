...
function runTests() {
  ...
  const actualLeak = calculateMonthlyLeak(testProfile1);
  ...
  const testProfileHighAC: Profile = {
    ...defaultProfile,
    acHours: 12,
  ...
  const actionsHighAC = createActions(testProfileHighAC, []);
  ...
  const testProfileNoAC: Profile = {
    ...defaultProfile,
    acHours: 0,
  };
  
  const actionsNoAC = createActions(testProfileNoAC, []);
  ...
  try {
    ...
    if (lightThemeBlock) {
      ...
      if (blockBgMatch) lightBg = blockBgMatch[1];
      if (blockFgMatch) lightFg = blockFgMatch[1];
    }
